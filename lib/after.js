...
afterEach( 'Sauce Breakpoint', function() {
...
afterEach( 'Dismiss any alerts for switching away', function() {
	this.timeout( afterHookTimeoutMS );
	const driver = global.__BROWSER__;
...
afterEach( 'Check for console errors', function() {
	if ( global.__MOBILE__ ) {
		return;
	}
...
afterEach( 'Update Sauce Job Status locally', function() {
	const driver = global.__BROWSER__;
...
after( 'Push SauceLabs job status update (if applicable)', function() {
	this.timeout( afterHookTimeoutMS );
	const driver = global.__BROWSER__;
