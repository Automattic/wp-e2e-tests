...
export default class SignUpFlow {
	constructor( driver, { blogName, expectedBlogAddresses, emailAddress, password } ) {
		...
		this.expectedBlogAddresses =
			expectedBlogAddresses || dataHelper.getExpectedFreeAddresses( this.blogName );
	...
	signupFreePlan() {
		return driverManager
			...
			.then( () => {
				...
				return new AboutPage( this.driver ).submitForm();
			} )
			.then( () => {
			...
			.then( () =>
				this.findADomainComponent.checkAndRetryForFreeBlogAddresses(
					this.expectedBlogAddresses,
					this.blogName
				)
			)
			...
			.then( () => {
				...
				return this.createYourAccountPage.enterAccountDetailsAndSubmit(
					this.emailAddress,
					this.blogName,
					this.password
				);
			...
			.then( () => {
				return new ViewBlogPage( this.driver ).waitForTrampolineWelcomeMessage();
			} )
