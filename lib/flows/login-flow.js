...
import GutenbergPage from '../pages/gutenberg-page';
...
export default class LoginFlow {
	...
	async loginAndStartNewPost( siteURL = null, usingGutenberg = false ) {
		if (
			siteURL ||
		...
		if ( isCalypsoIntegration ) {
			return await GutenbergPage.Visit( this.driver );
		}

		await ReaderPage.Expect( this.driver );
		let navbarComponent = await NavBarComponent.Expect( this.driver );

		navbarComponent = await NavBarComponent.Expect( this.driver );
