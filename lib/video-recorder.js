...
export function startVideo() {
	if ( process.env.TEST_VIDEO === 'true' ) {
		...
		const fileName = `${global.displayNum}-${dateTime}.mp4`;
		...
		console.log( 'created ff' + ffmpeg.pid );
	}
}
...
export function stopVideo( currentTest = null ) {
	if ( process.env.TEST_VIDEO === 'true' ) {
		...
		if ( currentTest && ffmpeg ) {
			...
			console.log( 'killelseif' + ffmpeg.pid );
			ffmpeg.kill();
