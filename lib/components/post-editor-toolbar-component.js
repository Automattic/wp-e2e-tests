...
export default class PostEditorToolbarComponent extends BaseContainer {
	...
	async publishAndPreviewPublished( { useConfirmStep = false } = {} ) {
		...
		if ( useConfirmStep === true ) {
			...
			await this.editorConfirmationSidebarComponent.confirmAndPublish();
		}
		await this.waitForSuccessViewPostNotice();
		return await this.previewPublishedPostOrPage();
	}
	...
	async publishThePost( { useConfirmStep = false } = {} ) {
		let result = await this.clickPublishPost();
		if ( useConfirmStep === true ) {
			this.editorConfirmationSidebarComponent = new EditorConfirmationSidebarComponent(
				this.driver
			);
			return await this.editorConfirmationSidebarComponent.confirmAndPublish();
		}
		return result;
	}
