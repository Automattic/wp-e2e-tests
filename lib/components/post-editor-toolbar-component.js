...
export default class PostEditorToolbarComponent extends BaseContainer {
	...
	publishAndPreviewPublished( { useConfirmStep = false } = {} ) {
		...
		// 	new EditorConfirmationSidebarComponent(
		// 		this.driver
		// 	).confirmAndPublish();
		// }
		this.publishThePost( { useConfirmStep } );
	...
	publishAndViewContent( { reloadPageTwice = false, useConfirmStep = false } = {} ) {
		...
		new PostPreviewComponent( this.driver ).edit();
	...
	publishThePost( { useConfirmStep = false } = {} ) {
		this.clickPublishPost();
		if ( useConfirmStep === true ) {
			new EditorConfirmationSidebarComponent( this.driver ).confirmAndPublish();
		}
	}
	...
	statusIsPending() {
		return this.driver
			...
			.getAttribute( 'class' )
			.then( classNames => classNames.includes( 'is-pending' ) );
	}
	...
	statusIsDraft() {
		return this.driver
			.findElement( By.css( '.editor-status-label' ) )
			.getAttribute( 'class' )
			.then( classNames => classNames.includes( 'is-draft' ) );
	}
