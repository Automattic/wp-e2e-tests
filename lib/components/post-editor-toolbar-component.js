...
export default class PostEditorToolbarComponent extends BaseContainer {
	constructor( driver ) {
		...
		this.successNoticeSelector = By.css( '.post-editor__notice.is-success,.post-editor-notice.is-success,.notice.is-success,.post-editor-notice.is-success' );
	}

	ensureSaved( { clickSave = true } = {} ) {
		const onMobile = driverManager.currentScreenSize() === 'mobile';

		const mobileSaveSelector = By.css( 'div.post-editor__content .editor-ground-control__status button.editor-ground-control__save' );
		const desktopSaveSelector = By.css( 'div.card.editor-ground-control .editor-ground-control__status button.editor-ground-control__save' );
