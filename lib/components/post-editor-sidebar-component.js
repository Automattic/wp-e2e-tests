...
export default class PostEditorSidebarComponent extends BaseContainer {
	...
	publishAndViewContent() {
		...
		driver.isElementPresent( viewPostSelector ).then( function( viewPostLocated ) {
			if ( ! viewPostLocated ) {
				driverHelper.clickWhenClickable( driver, publishButtonSelector, explicitWaitMS ); // sometimes first click doesnt work
			}
		} );
	...
	addNewCategory( category ) {
		const addNewCategoryButtonSelector = By.css( 'div.category-selector__add-category button.button' );
		const categoryNameInputSelector = By.css( 'div.category-selector__add-category-dialog input[type=text]' );
		const saveCategoryButtonSelector = By.css( 'div.category-selector__add-category-dialog button[data-reactid*=add]' );
		const driver = this.driver;
	...
	publicizeToTwitterAccountDisplayed() {
		const twitterAccountSelector = By.css( 'div.editor-sharing__publicize-connection[data-reactid*=twitter]' );
