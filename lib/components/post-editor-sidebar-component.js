...
export default class PostEditorSidebarComponent extends BaseContainer {
	constructor( driver ) {
		...
		this.usePublishConfirmation = config.get( 'usePublishConfirmation' );
		this.visibilitySelector = this.usePublishConfirmation ? By.css( '.editor-sidebar .editor-visibility__dropdown' ) : By.css( '.editor-drawer .editor-visibility button' );
	...
	setCommentsForPost( allow = true ) {
		...
		driverHelper.scrollElementIntoView( driver, selector );
		allow ? driverHelper.setCheckbox( driver, selector ) : driverHelper.unsetCheckbox( driver, selector );
	}
	...
	setVisibilityToPrivate() {
		const driver = this.driver;
		const valueSelector = this.usePublishConfirmation ? By.css( '.editor-sidebar a.select-dropdown__item[data-bold-text="private"]' ) : By.css( 'input[value=private]' );
	...
	setVisibilityToPasswordProtected( password ) {
		const driver = this.driver;
		const valueSelector = this.usePublishConfirmation ? By.css( '.editor-sidebar a.select-dropdown__item[data-bold-text="password"]' ) : By.css( 'input[value=password]' );
		const passwordEntrySelector = this.usePublishConfirmation ? By.css( '.editor-sidebar .editor-fieldset input[type="text"]' ) : By.css( 'div.editor-visibility__dialog input[type=text]' );
