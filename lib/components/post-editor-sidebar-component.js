...
export default class PostEditorSidebarComponent extends BaseContainer {
	...
	setVisibilityToPrivate() {
		...
		const driver = this.driver;
		this._expandOrCollapseSection( 'status', true );
		visibilitySelector = By.css( '.editor-sidebar .editor-visibility__dropdown' );
		driverHelper.clickWhenClickable( driver, visibilitySelector );
		driverHelper.clickWhenClickable( driver, By.css( '.editor-sidebar a.select-dropdown__item[data-bold-text="private"]' ) );
	...
	setVisibilityToPasswordProtected( password ) {
		...
		visibilitySelector = By.css( '.editor-sidebar .editor-visibility__dropdown' );
		driverHelper.clickWhenClickable( driver, visibilitySelector );
		driverHelper.clickWhenClickable( driver, By.css( '.editor-sidebar a.select-dropdown__item[data-bold-text="password"]' ) );
		return driverHelper.setWhenSettable( driver, By.css( '.editor-sidebar .editor-fieldset input[type="text"]' ), password, { secureValue: true } );
	}
