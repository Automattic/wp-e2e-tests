...
export default class ShoppingCartWidgetComponent extends BaseContainer {
	...
	displayed() {
		...
		driverHelper.isElementPresent( this.driver, by.className( 'cart-body' ) ).then( function( present ) {
			if ( present ) {
				d.fulfill( true );
			} else {
	...
	removeItem( self ) {
		...
		driverHelper.isElementPresent( this.driver, by.className( 'cart-empty' ) ).then( function( cartEmpty ) {
			if ( ! cartEmpty ) {
	...
	empty() {
		...
		driverHelper.isElementPresent( this.driver, by.className( 'popover-cart__count-badge' ) ).then( function( present ) {
			if ( present ) {
