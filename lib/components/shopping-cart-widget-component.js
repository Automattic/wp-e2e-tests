...
export default class ShoppingCartWidgetComponent extends BaseContainer {
	...
	open() {
		...
		this.displayedOpen().then( function cartAlreadyOpen() {
			d.fulfill( true );
		}, function cartClosed() {
			driverHelper.clickWhenClickable( driver, by.css( '.cart-toggle-button' ), explicitWait ).then(
			function success() {
				d.fulfill( true );
			}, function failure() {
	...
	displayedOpen() {
		...
		driverHelper.isElementPresent( this.driver, by.css( 'cart-body' ) ).then( function( present ) {
			if ( present ) {
				d.fulfill( true );
			} else {
	...
	removeItem( self ) {
		...
		driverHelper.isElementPresent( this.driver, by.css( '.cart-empty' ) ).then( function( cartEmpty ) {
			if ( ! cartEmpty ) {
				driverHelper.clickWhenClickable( self.driver, by.css( '.cart-item .remove-item' ) );
			}
		} );
		...
		driverHelper.isElementPresent( this.driver, cartBadgeSelector ).then( function( present ) {
			if ( present ) {
				self.open().then( function() {
					self.driver.findElement( cartBadgeSelector ).getText().then( function( numItems ) {
