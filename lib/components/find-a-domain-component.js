...
export default class FindADomainComponent extends BaseContainer {
	...
	selectDomainAddress( domainAddress ) {
		...
		this.driver.wait( until.elementLocated( selector ), this.explicitWaitMS, 'Could not locate the select button for the paid address: ' + domainAddress );
		const element = this.driver.findElement( selector );
		this.driver.wait( until.elementIsEnabled( element ), this.explicitWaitMS, 'The paid address button for ' + domainAddress + ' does not appear to be enabled to click' );
