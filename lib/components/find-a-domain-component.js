...
export default class FindADomainComponent extends BaseContainer {
	...
	selectDotComAddress( dotComAddress ) {
		//const selector = By.css( `button[data-domain="${ dotComAddress }"]` ); // if https://github.com/Automattic/wp-calypso/pull/5409 is merged
		const selector = By.xpath( `//*[text()='${ dotComAddress }']/ancestor::div[contains(concat(' ', @class, ' '), ' domain-suggestion ')]//div[contains(concat(' ', @class, ' '), ' domain-suggestion__action ')]/button` ); //nastiest xpath known to mankind
