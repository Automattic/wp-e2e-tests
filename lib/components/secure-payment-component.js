...
export default class SecurePaymentComponent extends BaseContainer {
	...
	enterTestCreditCardDetails( { cardHolder, cardNumber, cardExpiry, cardCVV, cardCountryCode, cardPostCode } ) {
		...
		const pauseBetweenKeys = global.browserName === 'Internet Explorer' ? 1 : 0;

		driverHelper.setWhenSettable( this.driver, By.id( 'name' ), cardHolder, { pauseBetweenKeys: pauseBetweenKeys } );

		// But why?...
		driverHelper.waitForFieldClearable( this.driver, By.id( 'number' ) );
		this.driver.findElement( By.id( 'number' ) ).sendKeys( cardNumber );

		driverHelper.setWhenSettable( this.driver, By.id( 'expiration-date' ), cardExpiry, { pauseBetweenKeys: pauseBetweenKeys } );
		driverHelper.setWhenSettable( this.driver, By.id( 'cvv' ), cardCVV, { pauseBetweenKeys: pauseBetweenKeys } );
