...
export default class SecurePaymentComponent extends BaseContainer {
	...
	enterTestCreditCardDetails( { cardHolder, cardNumber, cardExpiry, cardCVV, cardCountryCode, cardPostCode } ) {
		...
		// The CC# field doesn't provide accurate status back, so we don't verify the contents after setting
		driverHelper.setWhenSettable( this.driver, By.id( 'number' ), cardNumber, { pauseBetweenKeysMS: pauseBetweenKeysMS, assumeSuccess: true } );
