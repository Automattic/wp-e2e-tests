	...
	waitForPageToDisappear() {
		...
		return driver.wait( function() {
			...
			driver.findElement( By.css( '.notice.is-error' ) ).then( ( element ) => {
				this.errorShown = element.getText();
			}, ( e ) => {
				this.errorShown = error + e;
			} );
			const message = `Found issue on payment details page: '${this.errorShown}' - Submitting payment details again now.`;
			console.log( message );
