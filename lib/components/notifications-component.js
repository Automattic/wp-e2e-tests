...
export default class NotificationsComponent extends BaseContainer {
	...
	trashComment() {
		...
		this.switchToiFrame();
		this.driver.wait( until.elementLocated( trashPostSelector ), this.explicitWaitMS, 'Could not locate the trash comment button' );
		const trashPostElement = this.driver.findElement( trashPostSelector );
		this.driver.wait( until.elementIsVisible( trashPostElement ), this.explicitWaitMS, 'The trash post comment is not visible' );
