...
import { By as by, until, promise } from 'selenium-webdriver';
...
export default class NotificationsComponent extends AsyncBaseContainer {
	...
	// 	const commentSelector = by.css( `.wpnc__excerpt[data-e2e-comment="${ commentText }"]` );
	// 	return await driverHelper.clickWhenClickable( this.driver, commentSelector );
	// }

	async selectCommentByText( commentText ) {
		const commentSelector = async () => {
			let allComments = await this.driver.findElements( by.css( '.wpnc__excerpt' ) );
			return await promise.filter( allComments, async c => ( await c.getText() ) === commentText );
		};
		return await driverHelper.clickWhenClickable( this.driver, commentSelector );
	}
