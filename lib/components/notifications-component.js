...
export default class NotificationsComponent extends BaseContainer {
	...
	selectCommentByText( commentText ) {
		const self = this;
		const commentSelector = by.css( '.excerpt' );
		this.switchToiFrame();
		this.driver.findElement( by.css( commentSelector ) ).getText().then( function( text ) {
			if ( text === commentText ) {
				driverHelper.clickWhenClickable( self.driver, commentSelector );
