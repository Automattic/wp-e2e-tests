...
export default class SidebarComponent extends AsyncBaseContainer {
	...
	async selectAddPerson() {
		const selector = By.css( '.sites-navigation [data-tip-target="users"] a.sidebar__button' );
		await driverHelper.scrollIntoView( this.driver, selector );
		return await driverHelper.clickWhenClickable( this.driver, selector );
	}

	async selectManagePlugins() {
		const selector = By.css( '.sites-navigation [data-tip-target="plugins"] a.sidebar__button' );
		return await driverHelper.clickWhenClickable( this.driver, selector );
	}
	...
	async searchForSite( searchString ) {
		...
		const searchEnabled = await driverHelper.isElementPresent( this.driver, searchSelector );
		if ( searchEnabled ) {
	...
	async selectPlugins() {
		const selector = By.css( '.sites-navigation [data-tip-target="plugins"] a' );
