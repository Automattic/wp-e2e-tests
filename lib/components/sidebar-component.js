...
export default class SidebarComponent extends BaseContainer {
	...
	searchForSite( searchString ) {
		...
		return this.driver.findElement( searchSelector ).then( searchField =>
			searchField.isDisplayed().then( searchEnabled => {
				if ( searchEnabled ) {
					return driverHelper
	...
	selectPlugins() {
		...
		return driverHelper
			...
			.then(
				...
				() =>
					driverHelper
						.clickIfPresent( this.driver, dismissNoticeSelector )
	...
	selectSettings() {
		...
		return driverHelper
			.clickWhenClickable( this.driver, this.settingsSelector )
			.then(
				() => {},
				() =>
					driverHelper
	...
	numberOfMenuItems() {
		return this.driver
			.findElements( By.css( '.sidebar .sidebar__menu li' ) )
					...
					.then( foundBroken => {
						...
						return driverHelper
							...
							.then( () => {
								...
								new DisconnectSurveyPage( this.driver )
