...
export default class SidebarComponent extends BaseContainer {
	...
	selectSiteSwitcher() {
		...
		return driverHelper
			.isEventuallyPresentAndDisplayed( this.driver, siteSwitcherSelector, 1000 )
			.then( present => {
				if ( present ) {
	...
	searchForSite( searchString ) {
		...
		.then( searchField => searchField.isDisplayed()
		.then( searchEnabled => {
			if ( searchEnabled ) {
				return driverHelper
					.setWhenSettable( this.driver, searchSelector, searchString )
					.then( () => driverHelper.clickWhenClickable( this.driver, siteSelector ) );
			}
	...
	selectPlugins() {
		...
		return driverHelper
			.clickWhenClickable( this.driver, selector )
			.then(
				() => {},
				() => driverHelper.clickIfPresent( this.driver, dismissNoticeSelector )
					.then( () => driverHelper.clickWhenClickable( this.driver, selector ) )
			)
			.then( () => this.driver.wait( until.elementLocated( By.css( '.plugins-browser-list' ) ) ) );
	}
	selectSettings() {
		let dismissNoticeSelector = By.css( '.notice.is-dismissable .notice__dismiss' );

		return driverHelper.clickWhenClickable( this.driver, this.settingsSelector ).then(
			() => {},
			() => driverHelper.clickIfPresent( this.driver, dismissNoticeSelector )
			.then( () => driverHelper.clickWhenClickable( this.driver, this.settingsSelector ) )
		);
	}
	...
	numberOfMenuItems() {
		return this.driver.findElements( By.css( '.sidebar .sidebar__menu li' ) ).then( elements => elements.length );
	}
	...
	removeBrokenSite() {
		...
		this.ensureSidebarMenuVisible();
		return driverHelper
			.isElementPresent( this.driver, siteSwitcherSelector )
			.then( foundSwitcher => {
				if ( ! foundSwitcher ) {
				...
				this.selectSiteSwitcher();
				return driverHelper
					.isElementPresent( this.driver, clearSearchButton )
					.then( clearSearch => {
						if ( clearSearch ) {
							driverHelper.clickWhenClickable( this.driver, clearSearchButton );
						}
						return driverHelper.isElementPresent( this.driver, brokenSiteButton );
					} )
					.then( foundBroken => {
						if ( ! foundBroken ) {
							// no broken sites
						...
						return driverHelper
							.waitTillPresentAndDisplayed( this.driver, disconnectJetpackButton )
							.then( () => {
								driverHelper.clickWhenClickable( this.driver, disconnectJetpackButton );
								new DisconnectSurveyPage( this.driver ).skipSurveyAndDisconnectSite().then( () => true );
							} );
					} );
			} );
	}
