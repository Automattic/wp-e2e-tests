	...
	removeBrokenSite() {
		...
		return driverHelper.isElementPresent( this.driver, siteSwitcherSelector ).then( foundSwitcher => {
			...
			return driverHelper.isElementPresent( this.driver, brokenSiteButton ).then( foundBroken => {
				...
				return driverHelper.clickWhenClickable( this.driver, brokenSiteButton )
				.then( () => {
					return driverHelper.waitTillPresentAndDisplayed(
						this.driver,
						disconnectJetpackButton
					).then( () => {
						return driverHelper.clickWhenClickable( this.driver, disconnectJetpackButton )
						.then( () => {
							return driverHelper.waitTillPresentAndDisplayed(
								this.driver,
								removeSiteButton
	...
	removeAllBrokenSites() {
		...
		return driverHelper.isElementPresent( this.driver, siteSwitcherSelector ).then( foundSwitcher => {
			...
			return driverHelper.isElementPresent( this.driver, brokenSiteButton ).then( foundBroken => {
				...
				return driverHelper.isEventuallyPresentAndDisplayed( this.driver, brokenSiteButton ).then( () => {
					return this.driver.findElements( brokenSiteButton ).then( brokenSites => {
						return this.selectAllSites().then( () => {
							return driverHelper.isEventuallyPresentAndDisplayed( this.driver, brokenSiteButton ).then( () => {
								return brokenSites.forEach( () => {
									debug( 'removing site id: ' + count );
									return this.removeBrokenSite();
								} );
							} );
						} );
					} );
