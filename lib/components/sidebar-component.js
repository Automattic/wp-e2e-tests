...
export default class SidebarComponent extends BaseContainer {
	...
	ensureSidebarMenuVisible() {
		...
		return this.driver
			...
			.then(
				sidebarVisible =>
					! sidebarVisible
						? driverHelper.clickWhenClickable( this.driver, allSitesSelector )
						: false
			)
			.then( () => driverHelper.waitTillPresentAndDisplayed( this.driver, sidebarSelector ) );
	}
	...
	removeBrokenSite() {
		...
		this.ensureSidebarMenuVisible();
		return driverHelper
			.isEventuallyPresentAndDisplayed( this.driver, siteSwitcherSelector )
			.then( foundSwitcher => {
				if ( ! foundSwitcher ) {
