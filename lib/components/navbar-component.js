...
export default class NavbarComponent extends BaseContainer {
	...
	clickCreateNewPost( { siteURL = null } = {} ) {
		...
		return driverHelper.clickWhenClickable( this.driver, postButtonSelector ).then( () => {
			if ( siteURL !== null ) {
	...
	hasUnreadNotifications() {
		return this.driver
			...
			.getAttribute( 'class' )
			.then( classNames => classNames.includes( 'has-unread' ) );
	}
	openNotifications() {
	...
	dismissGuidedTours() {
		...
		return driverHelper
			.isElementPresent( this.driver, guidedToursDialogSelector )
			.then( present => {
				if ( present === true ) {
					return driverHelper.clickWhenClickable( this.driver, guidedToursDismissButtonSelector );
				}
			} );
	}
}
