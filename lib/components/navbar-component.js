...
export default class NavbarComponent extends BaseContainer {
	...
	clickCreateNewPost( { siteURL = null } = {} ) {
		...
		return driverHelper.clickWhenClickable( this.driver, postButtonSelector, this.explicitWaitMS ).then( () => {
			if ( siteURL !== null ) {
				return driverHelper.clickIfPresent( this.driver, by.xpath( `//div[text()="${siteURL}"]` ) );
			}
		} );
	}
	clickProfileLink() {
	...
	dismissGuidedTours() {
		...
		const guidedToursDismissButtonSelector = by.css( 'div.guided-tours__step-first button:not(.is-primary)' );
		return driverHelper.isElementPresent( self.driver, guidedToursDialogSelector ).then( ( present ) => {
			if ( present === true ) {
