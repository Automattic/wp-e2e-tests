...
export default class SiteViewComponent extends BaseContainer {
	constructor( driver ) {
		super( driver, by.css( '.web-preview__frame' ) );
	}
	isWebPreviewPresent() {
		this.driver.switchTo().defaultContent();
		return driverHelper.isElementPresent( this.driver, by.css( '.web-preview__external' ) );
	}
	isViewportSizeSelectorPresent() {
		this.driver.switchTo().defaultContent();
		return driverHelper.isElementPresent( this.driver, by.css( '.web-preview__toolbar .web-preview__device-switcher' ) );
	}
	isSitePresent() {
