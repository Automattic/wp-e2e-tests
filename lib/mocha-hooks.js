...
before( async function() {
	return videoRecorder.startDisplay();
} );

// Start xvfb display and recording
before( async function() {
	return videoRecorder.startVideo();
} );

// Stop video recording if the test has failed
afterEach( async function() {
	if ( this.currentTest.state === 'failed' ) {
		return videoRecorder.stopVideo( this.currentTest );
	}
} );
...
// Stop recording
after( function() {
	return videoRecorder.stopVideo();
} );
...
// Stop xvfb display
after( function() {
	return videoRecorder.stopDisplay();
} );
