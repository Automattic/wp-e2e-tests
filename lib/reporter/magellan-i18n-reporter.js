...
Reporter.prototype.listenTo = function( testRun, test, source ) {
	...
	fs.stat( './screenshots-i18n', function( err ) {
		if ( err && err.code === 'ENOENT' ) {
			fs.mkdir( './screenshots-i18n', function() {} );
		}
	} );
	...
	source.on( 'message', function( msg ) {
		if ( msg.type === 'worker-status' ) {
			...
			const screenshotDir = testRun.tempAssetPath + '/screenshots-i18n';
			...
			if ( passCondition || failCondition ) {
				...
				fs.readdir( screenshotDir, function( dirErr, screenshotFiles ) {
					...
					screenshotFiles.forEach( function( screenshotPath ) {
						...
						/// Move to /screenshots-i18n for CircleCI artifacts
						try {
							fs.rename( `${screenshotDir}/${screenshotPath}`, `./screenshots-i18n/${screenshotPath}`, function( moveErr ) {
								if ( moveErr ) {
									slackClient.send( {
										icon_emoji: ':a8c:',
										text: `Build <https://circleci.com/gh/${process.env.CIRCLE_PROJECT_USERNAME}/${process.env.CIRCLE_PROJECT_REPONAME}/${process.env.CIRCLE_BUILD_NUM}|#${process.env.CIRCLE_BUILD_NUM}> Moving file to /screenshots-i18n failed: '${moveErr}'`,
										username: 'e2e Test Runner'
									} );
								}
							} );
