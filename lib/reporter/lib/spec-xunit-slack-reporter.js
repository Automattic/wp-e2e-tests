...
function SpecXUnitSlack( runner ) {
	...
	if ( config.has( 'slackHook' ) && process.env.CIRCLE_BRANCH === 'master' ) {
		...
		runner.on( 'fail', function( test, err ) {
			...
			slackClient.send( {
				icon_emoji: ':a8c:',
				text: '<!here> Test Failed: *' + test.fullTitle() + '* - Build <https://circleci.com/gh/' + process.env.CIRCLE_PROJECT_USERNAME + '/' + process.env.CIRCLE_PROJECT_REPONAME + '/' + process.env.CIRCLE_BUILD_NUM + '|#' + process.env.CIRCLE_BUILD_NUM + '>',
				fields: fieldsObj,
