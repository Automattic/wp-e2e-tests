	...
	if ( process.env.CIRCLE_BRANCH === 'master' ) {
		...
		if ( target && config.has( target ) ) {
			...
			if ( targetConfig.has( 'slackHook' ) ) {
				slackHook = targetConfig.get( 'slackHook' );
			}
		} else if ( config.has( 'slackHook' ) ) {
			slackHook = config.get( 'slackHook' );
		}
				...
				fs.readdir( reportDir, function( dirErr, files ) {
					...
					files.forEach( function( reportPath ) {
						...
						try {
							...
							const failures = xmlData[ 0 ].tests.filter( t => t.status === 'fail' );
							...
							failures.forEach( function( failure ) {
								...
								slackClient.send( {
									icon_emoji: ':a8c:',
									text: `<!subteam^S0G7K98MB|flow-patrol-squad-team> Test Failed (${
										process.env.BROWSERSIZE
									}): *${ testName }* - Build <https://circleci.com/gh/${
										process.env.CIRCLE_PROJECT_USERNAME
									}/${ process.env.CIRCLE_PROJECT_REPONAME }/${ process.env.CIRCLE_BUILD_NUM }|#${
										process.env.CIRCLE_BUILD_NUM
									}>`,
								...
								try {
									fs.rename(
										...
										`./reports/${ testRun.guid }_${ reportPath }`,
										moveErr => {
											if ( ! moveErr ) {
												return;
											}
											...
											err => {
												...
												slackClient.send( {
													...
													text: `Build <https://circleci.com/gh/${
														...
														process.env.CIRCLE_BUILD_NUM
													}|#${ process.env.CIRCLE_BUILD_NUM }> Upload to slack failed: '${ err }'`,
													username: 'e2e Test Runner',
												} );
											}
										);
								...
								fs.rename( `${ dir }/${ path }`, `./screenshots/${ path }`, moveErr => {
									...
									slackClient.send( {
										...
										text: `Build <https://circleci.com/gh/${
											...
											process.env.CIRCLE_BUILD_NUM
										}> Moving file to /screenshots failed: '${ moveErr }'`,
										username: 'e2e Test Runner',
									} );
								} );
							} catch ( e ) {
					...
					screenshotFiles.forEach( function( screenshotPath ) {
						...
						try {
							fs.rename(
								...
								`${ finalScreenshotDir }/${ screenshotPath }`,
								moveErr => {
									if ( ! moveErr ) {
										return;
									}
