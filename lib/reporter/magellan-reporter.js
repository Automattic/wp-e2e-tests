...
Reporter.prototype.listenTo = function( testRun, test, source ) {
	if ( config.has( 'slackHook' ) && process.env.CIRCLE_BRANCH === 'try/magellan-reporter' ) {
		...
		source.on( 'message', function( msg ) {
			if ( msg.type === 'worker-status' ) {
				...
				if ( passCondition && test.attempts > 0 ) {
					slackClient.send( {
						icon_emoji: ':a8c:',
						text: 'FYI - The following test failed, retried, and passed: ' + testObject.title + ' - Build <https://circleci.com/gh/' + process.env.CIRCLE_PROJECT_USERNAME + '/' + process.env.CIRCLE_PROJECT_REPONAME + '/' + process.env.CIRCLE_BUILD_NUM + '|#' + process.env.CIRCLE_BUILD_NUM + '>',
					...
					fs.readdir( reportDir, function( dirErr, files ) {
						...
						files.forEach( function( reportPath ) {
							...
							failures.forEach( function( failure ) {
								...
								slackClient.send( {
									icon_emoji: ':a8c:',
									text: '<!subteam^S0G7K98MB|flow-patrol-squad-team> Test Failed: *' + testName + '* - Build <https://circleci.com/gh/' + process.env.CIRCLE_PROJECT_USERNAME + '/' + process.env.CIRCLE_PROJECT_REPONAME + '/' + process.env.CIRCLE_BUILD_NUM + '|#' + process.env.CIRCLE_BUILD_NUM + '>',
									fields: fieldsObj,
									username: 'e2e Test Runner'
								} );
							} );
						...
						files.forEach( function( screenshotPath ) {
							...
							slackUpload.uploadFile( {
								...
								if ( err ) {
									slackClient.send( {
										icon_emoji: ':a8c:',
										text: `Upload to slack failed: '${err}'`,
										username: 'e2e Test Runner'
									} );
								} else {
									console.log( 'done' );
				...
				if ( passCondition || failCondition ) {
					...
					fs.readdir( reportDir, function( dirErr, reportFiles ) {
						...
						reportFiles.forEach( function( reportPath ) {
							...
							fs.rename( `${reportDir}/${reportPath}`, `./reports/${reportPath}`, function( moveErr ) {
								if ( moveErr ) {
									slackClient.send( {
										icon_emoji: ':a8c:',
										text: `Moving file to /reports failed: '${moveErr}'`,
										username: 'e2e Test Runner'
									} );
								}
							} );
						...
						screenshotFiles.forEach( function( screenshotPath ) {
							...
							fs.rename( `${screenshotDir}/${screenshotPath}`, `./screenshots/${screenshotPath}`, function( moveErr ) {
								if ( moveErr ) {
									slackClient.send( {
										icon_emoji: ':a8c:',
										text: `Moving file to /screenshots failed: '${moveErr}'`,
										username: 'e2e Test Runner'
									} );
								}
							} );
