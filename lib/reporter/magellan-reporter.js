...
Reporter.prototype.listenTo = function( testRun, test, source ) {
	...
	source.on( 'message', function( msg ) {
		if ( msg.type === 'worker-status' ) {
			...
			if ( failCondition ) {
				// Reports
				// sendNotifPerFailure( slackClient, reportDir, testRun );
...
// function sendNotifPerFailure( slackClient, reportDir, testRun ) {
...
// 			failures = xmlData[0].tests.filter( ( t ) => t.status === 'fail' );
// 		} catch ( e ) {
