...
export default class FacebookPage extends BaseContainer {
	...
	checkPostWithPhotoDisplayed( messageText ) {
		...
		return driver.wait( function() {
			...
			slackNotifier.warn( `The Facebook page does not contain the expected post text ('${messageText}') with an image after waiting for it to appear. Please manually check that this eventually appears.` );
		} );
	}
	...
	checkPostWithTextDisplayed( messageText ) {
		...
		return driver.wait( function() {
			...
			slackNotifier.warn( `The Facebook page does not contain the expected post text ('${messageText}') after waiting for it to appear. Please manually check that this eventually appears.` );
		} );
	}
