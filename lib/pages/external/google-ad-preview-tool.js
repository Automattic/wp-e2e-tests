...
export default class GoogleAdPreviewTool extends BaseContainer {
	...
	setComDomain( language ) {
		var d = webdriver.promise.defer();
		this.driver.findElement( by.id( 'gwt-debug-diagnose-keywords-domains-widgetv2' ) ).click();
		this.driver.findElement( by.xpath( "//div[contains(text(), 'com')]" ) ).click();
		d.fulfill( true );
		return d.promise;
	}
	search( searchTerm ) {
