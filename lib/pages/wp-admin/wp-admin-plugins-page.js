...
export default class WPAdminPluginsPage extends BaseContainer {
	...
	JetpackVersionInstalled() {
		return this.driver.findElement( by.css( 'tr[data-slug="jetpack"] .plugin-version-author-uri,tr[data-slug="jetpack"] .plugin-version-author-uri' ) ).getText().then( ( txt ) => {
	...
	deactivateJetpack() {
		const self = this;
		const deactivateSelector = by.css( 'tr[data-slug="jetpack"] .deactivate,tr[data-slug="jetpack"] .deactivate' );
	...
	activateJetpack() {
		const self = this;
		const activateSelector = by.css( 'tr[data-slug="jetpack"] .activate,tr[data-slug="jetpack"] .activate' );
	...
	updateJetpack() {
		...
		const updateSelector = by.css( 'tr[data-slug="jetpack"] .activate,tr[data-slug="jetpack"] a.update-link' );
