...
export default class WPAdminAddPostPage extends BaseContainer {
	...
	publish() {
		...
		self.driver.sleep( 1000 ).then( () => { // The publish buttons takes a bit to enable
			driverHelper.clickWhenClickable( this.driver, publishSelector );
		} );
		return driverHelper.waitTillPresentAndDisplayed( this.driver, by.css( '#message.updated' ) );
	}
}
