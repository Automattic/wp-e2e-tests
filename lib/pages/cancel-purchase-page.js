...
export default class CancelPurchasePage extends AsyncBaseContainer {
	...
	async clickCancelPurchase() {
		const pauseBetweenClickAttemptsMS = 1000;
		...
		for ( let i = 0; i < this.explicitWaitMS / pauseBetweenClickAttemptsMS; i++ ) {
			await driverHelper.clickWhenClickable( this.driver, this.cancelButtonSelector );
			if ( await driverHelper.elementIsNotPresent( this.driver, this.cancelButtonSelector ) ) {
				return true;
			}
