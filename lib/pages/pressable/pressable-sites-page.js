...
export default class PressableSitesPage extends BaseContainer {
	...
	addNewSite( siteName ) {
		...
		return driverHelper
			.isElementPresent( this.driver, formSelector )
			.then( element => {
				if ( ! element ) {
			...
			.then( () => this.driver.sleep( 3000 ) ); // wait until new site is properly setted up
	}
	...
	deleteFirstSite() {
		...
		return driverHelper
			.clickWhenClickable( this.driver, settingsButton )
			.then( () => new PressableSiteSettingsPage( this.driver ).deleteSite() );
	}
}
