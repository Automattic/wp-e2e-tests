...
export default class PluginDetailsPage extends BaseContainer {
	...
	waitForPlugin() {
		return this.driver.wait(
			until.elementLocated( this.activatePluginSelector ),
			this.explicitWaitMS,
			'Could not locate the activate plugin link for the plugin'
		);
	}

	waitForSuccessNotice() {
		this.driver.wait(
			until.elementLocated( this.successNoticeSelector ),
			this.explicitWaitMS,
			'Could not locate the success notice. Check that is is displayed.'
		);
	}
	...
	ensureDeactivated() {
		return this.driver
			.findElement( this.activatePluginSelector )
			.getAttribute( 'aria-checked' )
			.then( active => {
				if ( active === 'true' ) {
					driverHelper.clickWhenClickable( this.driver, this.activatePluginSelector );
