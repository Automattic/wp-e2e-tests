...
export default class ThemesPage extends BaseContainer {
	...
	getCurrentThemeName() {
		const selector = by.css( '.current-theme__name' );
		driverHelper.waitTillPresentAndDisplayed( this.driver, selector );
		return this.driver.findElement( selector ).getText();
	}

	getCustomizeLink() {
		const selector = by.css( '.current-theme__customize' );
		driverHelper.waitTillPresentAndDisplayed( this.driver, selector );
		return this.driver.findElement( selector ).getAttribute( 'href' );
	}

	getInfoLink() {
		const selector = by.css( '.current-theme__info' );
		driverHelper.waitTillPresentAndDisplayed( this.driver, selector );
		return this.driver.findElement( selector ).getAttribute( 'href' );
	}
