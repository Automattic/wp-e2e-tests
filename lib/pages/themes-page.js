...
export default class ThemesPage extends BaseContainer {
	...
	showOnlyThemesType( type ) {
		driverHelper.clickWhenClickable( this.driver, by.css( `a[data-e2e-value="${type}"]` ) );
	...
	static _getThemeSelectionXpath( phrase ) {
		// return by.xpath( `//h2[text()[contains(.,'${ phrase }')]]//ancestor::div[contains(@class,'card')]` );
		//h2[text()[contains(.,'Twenty')]]//ancestor::div[contains(@class,'card') && not(contains(@class,'is-active'))]
		return by.xpath( `//h2[text()[contains(.,'${ phrase }')]]//ancestor::div[contains(@class,'card') and not(contains(@class,'is-active'))]` );
	}
}
