...
import { By as by, promise } from 'selenium-webdriver';
...
export default class ThemesPage extends AsyncBaseContainer {
	...
	async clickPopoverItem( name ) {
		const actionItem = async () => {
			let allItems = await this.driver.findElements( by.css( '.popover__menu-item' ) );
			return await promise.filter( allItems, async c => ( await c.getText() ) === name );
		};
		return await driverHelper.clickWhenClickable( this.driver, actionItem );
	}
