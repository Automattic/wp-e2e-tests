...
export default class ThemesPage extends BaseContainer {
	constructor( driver ) {
		...
		this.listLoadedSelector = by.className( 'themes-list-loaded' );
	}
	...
	showOnlyFreeThemes() {
		...
		driver.wait( until.elementLocated( this.listLoadedSelector ), this.explicitWaitMS, 'Themes List not fully populated'  ).then( function() {
	...
	getFirstThemeName() {
		...
		// Sleep to allow the search field to re-populate after a refresh
		return this.driver.wait( until.elementLocated( this.listLoadedSelector ), this.explicitWaitMS, 'Themes List not fully populated'  ).then( function() {
