...
export default class ThemesPage extends AsyncBaseContainer {
	...
	async clickPopoverItem( name ) {
		// Leave this comment just in case - For multi-sites it's 'button', for single sites it's 'a'.  WEIRD
		const previewThemeSelector = by.css( `.popover__menu-item[data-e2e-item="${ name }"]` );
		await driverHelper.waitTillPresentAndDisplayed( this.driver, previewThemeSelector );
