...
export class GetDotBlogContactForm extends BaseContainer {
	...
	inputDetails( personalInfo ) {
		...
		driverHelper.setWhenSettable( this.driver, by.id( 'email' ), personalInfo.email );
	...
	// Split out to induce all error messages
	inputState( stateCode ) {
		driverHelper.clickWhenClickable( this.driver, by.css( '.app-components-ui-contact-information-state select' ) );
		driverHelper.clickWhenClickable( this.driver, by.css( `.app-components-ui-contact-information-state select option[value="${stateCode}"]` ) );
		return this.driver.executeScript( `document.querySelector('.app-components-ui-contact-information-state select').value = '${stateCode}'` );
	}

	submitDetails() {
		return driverHelper.clickWhenClickable( this.driver, by.css( '.app-components-ui-button-button' ) );
	}

	clearEmail() {
		return driverHelper.setWhenSettable( this.driver, by.id( 'email' ), 'INVALID E-MAIL' );
	}

	clearNameAndPostalCode() {
		driverHelper.setWhenSettable( this.driver, by.id( 'name' ), '' );
		return driverHelper.setWhenSettable( this.driver, by.id( 'postalCode' ), 'ABC' );
	}
}
