...
export default class TwitterFeedPage extends BaseContainer {
	...
	checkLatestTweetsContain( expectedTweetText ) {
		...
		return driver.wait( function() {
			...
			slackNotifier.warn( `The twitter page does not contain the expected tweet text ('${expectedTweetText}') after waiting for it to appear. Please manually check that this eventually appears.` );
		} );
	}
	...
	checkTweetWithPhotoDisplayed( expectedTweetText ) {
		...
		return driver.wait( function() {
			...
			slackNotifier.warn( `The twitter page does not contain the expected tweet text ('${expectedTweetText}') and an image after waiting for it to appear. Please manually check that this eventually appears.` );
		} );
	}
