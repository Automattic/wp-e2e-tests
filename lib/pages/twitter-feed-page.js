...
export default class TwitterFeedPage extends BaseContainer {
	...
	checkTweetWithPhotoDisplayed( expectedTweetText ) {
		...
		if ( screenSize === 'mobile' ) {
			const xpath = `//div[contains(text(),"${expectedTweetText}")]/..//img[@alt="Embedded image"] | //*[contains(text(),"${expectedTweetText}")]/../../../..//img[@alt="Embedded"]`;
			console.log( `Using mobile twitter view xpath: '${xpath}'` );
			imageInTweetSelector = by.xpath( xpath );
		} else {
			const xpath = `//p[contains(text(),"${expectedTweetText}")]/../..//div[contains(@class, "AdaptiveMedia-photoContainer")]`;
			console.log( `Using desktop twitter view xpath: '${xpath}'` );
			imageInTweetSelector = by.xpath( xpath );
		}
		return driver.wait( function() {
			driver.navigate().refresh();
		...
		}, this.explicitWaitMS * 2, 'The twitter page does not contain the expected tweet text (' + expectedTweetText + ') and image after waiting for it to appear' );
	}
};
