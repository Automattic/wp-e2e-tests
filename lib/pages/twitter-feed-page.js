...
export default class TwitterFeedPage extends BaseContainer {
	...
	checkLatestTweetsContain( expectedTweetText ) {
		...
		}, this.explicitWaitMS * 2, 'The twitter page does not contain the expected tweet text (' + expectedTweetText + ') after waiting for it to appear' );
	}
	...
	checkTweetWithTextAppears( expectedTweetText ) {
		...
		}, this.explicitWaitMS * 3, 'The twitter page does not contain the expected tweet text (' + expectedTweetText + ') after waiting for it to appear' );
	}
	...
	checkTweetWithPhotoDisplayed( expectedTweetText ) {
		...
		}, this.explicitWaitMS * 3, 'The twitter page does not contain the expected tweet text (' + expectedTweetText + ') and image after waiting for it to appear' );
	}
