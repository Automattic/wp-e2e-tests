...
export default class ProfilePage extends BaseContainer {
	...
	chooseAccountSettings() {
		this._closeProfileViewOnMobile();
		return driverHelper.clickWhenClickable( this.driver, by.css( '.sidebar a[href$="account"]' ) );
	}
	...
	waitForProfileLinks() {
		...
		if ( process.env.VISDIFF ) {
			...
			driver.wait(
				function() {
					return driverHelper
						.isElementPresent( driver, linksLoadingSelector )
	...
	async _closeProfileViewOnMobile() {
		...
		let displayed = await driverHelper.isElementPresent( self.driver, by.css( '.focus-content' ) );
		if ( displayed ) {
			return await driverHelper.clickWhenClickable( self.driver, self.closeProfileViewSelector );
		}
	}
}
