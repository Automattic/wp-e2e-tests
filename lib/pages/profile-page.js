...
export default class ProfilePage extends BaseContainer {
	constructor( driver ) {
		...
		this.signOutSelector = by.css( '.sidebar__me-signout-button' );
	}

	clickSignOut() {
		this._closeProfileViewOnMobile();

		driverHelper.waitTillPresentAndDisplayed( this.driver, this.signOutSelector );
		return driverHelper.clickWhenClickable( this.driver, this.signOutSelector );
