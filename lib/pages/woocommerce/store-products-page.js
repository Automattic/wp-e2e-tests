...
export default class StoreProductsPage extends BaseContainer {
	constructor( driver, visit = false ) {
		const url = dataHelper.configGet( 'calypsoBaseURL' ) + '/store/products/' + config.get( 'wooCommerceSite' );
	...
	productDisplayed( productTitle ) {
		return driverHelper.isElementPresent( this.driver, StoreProductsPage._getSelectorForProduct( productTitle ) );
	}

	selectProduct( productTitle ) {
		return driverHelper.clickWhenClickable( this.driver, StoreProductsPage._getSelectorForProduct( productTitle ) );
	}
