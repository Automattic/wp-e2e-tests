...
export default class ViewSitePage extends BaseContainer {
	...
	followShareToTwitter() {
		...
		.then( href => this.driver.get( href ) ) );
	}

	firstPostTitle() {
		return this.driver.findElement( by.css( '.entry-title a' ) ).then( e => e.getAttribute( 'text' ) );
	}

	firstPostURL() {
		return this.driver.findElement( by.css( '.entry-title a' ) ).then( e => e.getAttribute( 'href' ) );
	}
