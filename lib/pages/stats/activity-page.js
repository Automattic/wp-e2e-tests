...
export default class ActivityPage extends BaseContainer {
	...
	addSiteCredentials() {
		...
		driverHelper.clickWhenClickable( this.driver, addCredentialsSelector );
		driverHelper.clickWhenClickable( this.driver, shareCredentialsSelector );
		driverHelper.clickWhenClickable( this.driver, viewActivitySelector );
		driverHelper.waitTillPresentAndDisplayed( this.driver, cloudIconSelector, explicitWaitMS * 7 );
		this.driver.getCurrentUrl().then( url => this.driver.get( url ) );
		return driverHelper.waitTillPresentAndDisplayed( this.driver, rewindButtonSelector );
	}
	...
	waitUntilBackupCompleted() {
		...
		driverHelper.waitTillPresentAndDisplayed( this.driver, cloudIconSelector );
		this.driver.getCurrentUrl().then( url => this.driver.get( url ) );
		return driverHelper.waitTillPresentAndDisplayed( this.driver, rewindButtonSelector );
	}
