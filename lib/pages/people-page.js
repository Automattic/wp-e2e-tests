...
export default class PeoplePage extends BaseContainer {
	...
	emptyUsers() {
		...
		driverHelper
			.isElementPresent( this.driver, By.css( '.people-list-section-header span.count' ) )
			.then( present => {
				if ( present ) {
					this.driver
						...
						.then( numItems => {
							...
							for ( let i = 0; i < parseInt( numItems.replace( /,/, '' ) ); i++ ) {
								promiseArray.push( flow.execute( () => this.removeUser( this ) ) );
							}
	...
	removeUser( self ) {
		...
		driverHelper
			.clickWhenClickable( self.driver, removeButton )
