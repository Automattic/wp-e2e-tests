...
export default class PeoplePage extends BaseContainer {
	...
	selectTeam() {
		...
		return DriverHelper.clickWhenClickable( this.driver, By.css( 'a[data-reactid*=team]' ) );
	}

	selectViewers() {
		this.ensureMobileMenuOpen();
		DriverHelper.clickWhenClickable( this.driver, By.css( 'a[data-reactid*=viewers]' ) );
		return this.waitForSearchResults();
	}

	selectEmailFollowers() {
		this.ensureMobileMenuOpen();
		return DriverHelper.clickWhenClickable( this.driver, By.css( 'a[data-reactid*=email-followers]' ) );
	}
