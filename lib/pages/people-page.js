...
export default class PeoplePage extends BaseContainer {
	...
	emptyUsers() {
		...
		DriverHelper.isElementPresent( this.driver, By.css( '.people-list-section-header span.count' ) ).then( function( present ) {
			if ( present ) {
	...
	viewerDisplayed( username ) {
		let viewers = this.driver.findElements( By.css( '.people-list-item[type="viewer"]' ) );
	...
	waitForSearchResults() {
		...
		return driver.wait( function() {
			return DriverHelper.isElementPresent( driver, searchResultsLoadingSelector ).then( ( present ) => {
				return !present;
			} );
