...
export default class WporgCreatorPage extends BaseContainer {
	constructor( driver, template = 'default' ) {
		...
		driverHelper.waitTillPresentAndDisplayed( driver, CONTINUE_LINK, this.explicitWaitMS * 7 );
	...
	getUrl() {
		driverHelper.waitTillPresentAndDisplayed( this.driver, URL_ELEMENT );
		return this.driver.findElement( URL_ELEMENT ).getText();
	}

	waitForWpadmin() {
		driverHelper.refreshIfJNError( this.driver );
