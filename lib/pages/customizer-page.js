...
var slackNotifier = require( '../slack-notifier.js' );

function CustomizerPage( driver ) {
	const saveSelector = by.css( '#save' );
	const reloadCustomizerSelector = by.css( '.empty-content__action.button' );
	this.driver = driver;
	...
	let self = this;
	driver.wait( until.elementLocated( this.iframeElementSelector ), this.explicitWaitMS * 2 ).then( function() { }, function( error ) {
		const message = `Found issue on customizer page: '${error}' - Clicking try again button now.`;
		slackNotifier.warn( message );
		driver.wait( function() {
			return driver.isElementPresent( reloadCustomizerSelector );
		}, self.explicitWaitMS ).then( function() {
			driverHelper.clickWhenClickable( driver, reloadCustomizerSelector, self.explicitWaitMS );
		}, function( err ) {
