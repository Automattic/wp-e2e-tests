...
export default class CustomizerPage extends BaseContainer {
	...
	waitForCustomizer() {
		const self = this;
		self.driver.wait( until.elementLocated( this.metaiFrameElementSelector ), this.explicitWaitMS * 2 ).then( function() {
		}, function( error ) {
	...
	selectActionSaveDraft() {
		return driverHelper.clickWhenClickable( this.driver, by.css( 'input[value="draft"]' ) );
	}
	...
	setHeaderImage( fileDetails ) {
		...
		}, this.explicitWaitMS, 'The uploading status is still present' );
	...
	createNewMenuAndSetAsHeader( menuName ) {
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();
		driverHelper.clickWhenClickable( this.driver, by.css( '#accordion-section-add_menu button.customize-add-menu-button' ) );
		driverHelper.setWhenSettable( this.driver, by.css( '#customize-control-add_menu-name input.menu-name-field' ), menuName );
		driverHelper.clickWhenClickable( this.driver, by.css( '#customize-new-menu-submit' ) );
		driverHelper.clickWhenClickable( this.driver, by.css( 'ul.menu.open input[data-location-id="menu-1"]' ) );
		this.waitRequestsToComplete();
	}

	addMenuItems( menuItemName1, menuItemName2 ) {
		driverHelper.setWhenSettable( this.driver, by.css( '#create-item-input-page' ), menuItemName1 );
		driverHelper.clickWhenClickable( this.driver, by.xpath( '//div[@id="available-menu-items-post_type-page"]//button[@class="button add-content"]' ) );
		this.waitRequestsToComplete();

		driverHelper.setWhenSettable( this.driver, by.css( '#create-item-input-page' ), menuItemName2 );
		driverHelper.clickWhenClickable( this.driver, by.xpath( '//div[@id="available-menu-items-post_type-page"]//button[@class="button add-content"]' ) );
		this.waitRequestsToComplete();
	}
	...
	navigateToPreviewURL() {
		this._switchToMetaiFrame();
		this.driver.get( this.getPreviewURL() );
		this.waitRequestsToComplete();
	}

	checkNavMenu( menuItemName1, menuItemName2 ) {//it's better to move it to separate class which refers to WP landing
		this.waitRequestsToComplete();
		if ( driverManager.currentScreenSize() === 'mobile' ) {
			driverHelper.waitTillPresentAndDisplayed( this.driver, by.xpath( '//button[@class="menu-toggle" or @class="secondary-toggle"]' ) );
			driverHelper.clickWhenClickable( this.driver, by.xpath( '//button[@class="menu-toggle" or @class="secondary-toggle"]' ) );
		}

		this.driver.findElement( by.xpath( '//*[@class="nav-menu"]//li[1]/a' ) ).getText().then( ( menu1 ) => {
			assert.equal( menu1.toLowerCase(), menuItemName1.toLowerCase() );
		} );

		this.driver.findElement( by.xpath( '//*[@class="nav-menu"]//li[2]/a' ) ).getText().then( ( menu2 ) => {
			assert.equal( menu2.toLowerCase(), menuItemName2.toLowerCase() );
		} );
	}

	openAddItemsSlider() {
		this.driver.sleep( 500 ); //waiting until animation is finished
		driverHelper.clickWhenClickable( this.driver, by.css( 'ul.menu.open button[class="button add-new-menu-item"]' ) );
		this.waitRequestsToComplete();
	}

	addNewMenuAndSetAsPrimaryAndClickBack( menuName ) {
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();
		driverHelper.clickWhenClickable( this.driver, by.css( '#accordion-section-add_menu button.customize-add-menu-button' ) );
		driverHelper.setWhenSettable( this.driver, by.css( '#customize-control-add_menu-name input.menu-name-field' ), menuName );
		driverHelper.clickWhenClickable( this.driver, by.css( '#customize-new-menu-submit' ) );
		driverHelper.clickWhenClickable( this.driver, by.css( 'ul.menu.open input[data-location-id="primary"]' ) );
		return driverHelper.clickWhenClickable( this.driver, this.backSelector );
	}

	openMenuSettingsByMenuName( menuName ) {
		return driverHelper.clickWhenClickable( this.driver, by.xpath( `//h3[contains(text(), "${ menuName }")]` ) );
	}
...
//		driverHelper.setWhenSettable( this.driver, by.css( 'li.expanded textarea.widefat' ), widgetContent );
	...
	menuDisplayed( menuName ) {
		let present = false;
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();

		present = this.driver.wait( until.elementLocated( by.xpath( `//h3[contains(text(), '${ menuName }')]` ) ), this.explicitWaitMS ).then( () => {
			return true;
		}, () => {
			return false;
		} );
	...
	waitForPreviewRefresh() {
		...
		return self.driver.wait( () => {
			return driverHelper.isElementPresent( self.driver, by.css( 'body.wp-customizer-unloading' ) );
	...
	clickDiscardChanges() {
		return driverHelper.clickWhenClickable( this.driver, by.css( '#customize-control-trash_changeset button' ) );
	}

	getPreviewURL() {
		return this.driver.findElement( by.css( '.preview-link-wrapper > a' ) ).getAttribute( 'href' );
	}

	waitRequestsToComplete() {
		return this.driver.wait( () => {
			return this.driver.executeScript( 'if (jQuery) {return jQuery.active === 0}' );
		}, this.explicitWaitMS, 'Timed out waiting while AJAX loads' );
	}
	...
	_ensureMetaViewOnMobile() {
		...
		if ( driverManager.currentScreenSize() === 'mobile' ) {
			this._switchToMetaiFrame();
			driverHelper.isElementPresent( driver, by.css( 'div.preview-desktop.preview-only' ) ).then( ( previewDisplayed ) => {
