...
export default class CustomizerPage extends BaseContainer {
		...
		this.driver
			...
			.then(
				...
				error => {
					...
					this.driver
						.wait( () => driverHelper.isElementPresent( this.driver, this.reloadCustomizerSelector ) )
						.then( () => driverHelper.clickWhenClickable(
							this.driver,
							this.reloadCustomizerSelector,
						),
							err => {
								console.log(
	...
	menuDisplayedAsPrimary( menuName ) {
		let present = false;
		const menuNameLocator = by.xpath( `//h3[contains(text(), '${ menuName }')]/span[contains(text(), 'Primary')]` );
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();

		present = this.driver
			.wait(
				until.elementLocated( menuNameLocator ),
				this.explicitWaitMS
			)
			.then(
				() => true,
