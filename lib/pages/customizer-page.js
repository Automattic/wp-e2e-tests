...
export default class CustomizerPage extends BaseContainer {
			...
			.then( present => {
				if ( present === false ) {
					...
					return driver
						...
						.then(
							() => {},
							err => slackNotifier.warn( `Error refreshing customizer: '${ err }'` )
						);
				}
			} );
		...
		this.driver
			...
			.then(
				...
				error => {
					...
					this.driver
						.wait( () =>
							driverHelper.isElementPresent( this.driver, this.reloadCustomizerSelector )
						)
						.then(
							() => driverHelper.clickWhenClickable( this.driver, this.reloadCustomizerSelector ),
							err => {
								console.log(
	...
	waitRequestsToComplete() {
		return this.driver.wait(
			() => this.driver.executeScript( 'if (jQuery) {return jQuery.active === 0}' ),
			this.explicitWaitMS,
	...
	selectActionSaveDraft() {
		...
		// return this.driver.switchTo().defaultContent();
		return this._clickInMetaiFrame( by.css( 'input[value="draft"]' ) );
	}
	...
	saveNewTheme() {
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();
		return this.driver.sleep( this.shortSleepMS ).then( () =>
			driverHelper.clickWhenClickable( this.driver, this.saveSelector ).then( () => {
				this.waitRequestsToComplete();
	...
	close() {
		return this._clickInMetaiFrame( by.css( '.customize-controls-close' ) );
	}
	...
	closeOpenSection() {
		...
		this._switchToMetaiFrame();
		this.driver
			.sleep( this.shortSleepMS )
			.then( () => driverHelper.clickWhenClickable( this.driver, this.backSelector ) )
			.then( () => this.driver.sleep( this.shortSleepMS ) )
			.then( () => driverHelper.isElementPresent( this.driver, this.backSelector ) )
			.then( present => {
				if ( present === true ) {
					slackNotifier.warn(
						'The close open section element is still present when it should have been clicked - trying to click again now'
					);
					return driverHelper.clickIfPresent( this.driver, this.backSelector );
				}
			} )
			.then( () => this._switchToDefaultContent() );
		return;
	}

	closeOpenPanel() {
		return this._clickInMetaiFrame( by.css( ".customize-panel-back[tabindex='0']" ) );
	}

	expandSiteIdentity() {
		return this._clickInMetaiFrame( by.css( 'li#accordion-section-title_tagline' ) );
	}

	expandColorsAndBackgrounds() {
		// this._ensureMetaViewOnMobile();
		// this._switchToMetaiFrame();
		// return this.driver.sleep( this.shortSleepMS ).then( () => driverHelper
		// .clickWhenClickable( this.driver, by.css( 'li#accordion-section-colors_manager_tool' ) )
		// .then( () => this._switchToDefaultContent() ) );
		return this._clickInMetaiFrame( by.css( 'li#accordion-section-colors_manager_tool' ) );
	}
	...
	expandFonts() {
		...
		// 		} );
		// } );
		return this._clickInMetaiFrame( by.css( 'li#accordion-section-jetpack_fonts' ) );
	}
	...
	expandMenus() {
		...
		// 		} );
		// } );
		return this._clickInMetaiFrame( by.css( 'li#accordion-panel-nav_menus' ) );
	}
	...
	expandWidgets() {
		...
		// 		} );
		// } );
		return this._clickInMetaiFrame( by.css( 'li#accordion-panel-widgets' ) );
	}
	...
	expandStaticFrontPage() {
		...
		// 		} );
		// } );
		return this._clickInMetaiFrame( by.css( 'li#accordion-section-static_front_page' ) );
	}
	...
	expandHeaderImage() {
		...
		// 		} );
		// } );
		return this._clickInMetaiFrame( by.css( 'li#accordion-section-header_image' ) );
	}
	...
	chooseFirstHeadingsFont() {
		...
		this.driver.wait(
			() => this.driver.findElement( firstFontSelector ).getText() !== '',
			this.explicitWaitMS,
	...
	setHeaderImage( fileDetails ) {
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();
		driverHelper.clickWhenClickable( this.driver, by.css( 'button#header_image-button' ) ); // add new image
		driverHelper.clickWhenClickable(
			this.driver,
			by.css( '.media-modal .media-router .media-menu-item' )
		); //upload files tab
		this.driver.findElement( by.css( 'input[type="file"]' ) ).sendKeys( fileDetails.file );
		this.driver.wait(
			() => driverHelper.elementIsNotPresent( this.driver, '.media-uploader-status.uploading' ),
			this.explicitWaitMS,
			'The uploading status is still present'
		);
		driverHelper.clickWhenClickable( this.driver, by.css( '.media-toolbar-primary button' ) ); //select and crop
		return driverHelper.clickWhenClickable( this.driver, by.css( 'button.media-button-insert' ) ); //crop image
	}
	...
	chooseStaticFrontPage() {
		...
		// return this._switchToDefaultContent();
		return this._clickInMetaiFrame( by.css( "input[type='radio'][value='page']" ) );
	}
	...
	clickSiteTitleIconInPreview() {
		...
		// return this._switchToDefaultContent();
		return this._clickInMetaiFrame( by.css( 'div.cdm-icon__blogname' ) );
	}
	...
	menuDisplayedAsPrimary( menuName ) {
		let present = false;
		const menuNameLocator = by.xpath(
			`//h3[contains(text(), '${ menuName }')]/span[contains(text(), 'Primary')]`
		);
		this._ensureMetaViewOnMobile();
		this._switchToMetaiFrame();

		present = this.driver
			.wait( until.elementLocated( menuNameLocator ), this.explicitWaitMS )
			.then( () => true, () => false );
		this._switchToDefaultContent();
		return present;
	}
	...
	previewTitle() {
		this._ensurePreviewViewOnMobile();
		this._switchToPreviewiFrame();
		const siteTitle = new ViewSitePage( this.driver ).siteTitle().then( title => title );
		this._switchToDefaultContent();
		return siteTitle;
	}
	...
	previewTagline() {
		...
		const siteTagLine = viewSitePage.siteTagline();
		this._switchToDefaultContent();
		return siteTagLine;
	}
	...
	waitForPreviewRefresh() {
		...
		return self.driver.wait(
			() => driverHelper.isElementPresent( self.driver, by.css( 'body.wp-customizer-unloading' ) ),
			self.explicitWaitMS,
	...
	_ensureMetaViewOnMobile() {
		if ( driverManager.currentScreenSize() === 'mobile' ) {
			this._switchToMetaiFrame();
			driverHelper
				.isElementPresent( this.driver, by.css( 'div.preview-desktop.preview-only' ) )
	...
	_clickInMetaiFrame( locator ) {
		...
		return this.driver
			...
			.then( () =>
				driverHelper
					.clickWhenClickable( this.driver, locator )
					.then( () => this._switchToDefaultContent() )
			);
	}
}
