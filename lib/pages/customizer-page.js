...
export default class CustomizerPage extends BaseContainer {
	...
	checkNavMenu( menuItemName1, menuItemName2 ) {//it's better to move it to separate class which refers to WP landing
		if ( driverManager.currentScreenSize() === 'mobile' ) {
			driverHelper.clickWhenClickable( this.driver, by.css( '.secondary-toggle' ) );
		}
		if ( driverManager.currentScreenSize() === 'desktop' ) {
			menuItemName1 = menuItemName1.toUpperCase();
			menuItemName2 = menuItemName2.toUpperCase();
		}
		this.driver.findElement( by.xpath( '//*[@class="nav-menu"]//li[1]/a' ) ).getText().then( ( menu1 ) => {
			assert.equal( menu1, menuItemName1 );
		} );

		this.driver.findElement( by.xpath( '//*[@class="nav-menu"]//li[2]/a' ) ).getText().then( ( menu2 ) => {
			assert.equal( menu2, menuItemName2 );
		} );
	}
