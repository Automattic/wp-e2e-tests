...
export default class StartPage extends BaseContainer {
	constructor( driver, { visit = true, culture = 'en', flow = '', domainFirst = false, domainFirstDomain = '', vertical = '' } = {} ) {
		let url;
		if ( visit === true ) {
			let queryStrings = '';
			...
			if ( vertical !== '' ) {
				queryStrings = StartPage.appendQueryString( queryStrings, `vertical=${ vertical }` );
			}
