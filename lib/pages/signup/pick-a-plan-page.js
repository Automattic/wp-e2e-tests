...
export default class PickAPlanPage extends BaseContainer {
	...
	_selectPlan( level ) {
		...
		const selector = `${prefix} button.is-${level}-plan`;

		driverHelper.waitTillNotPresent( this.driver, By.css( selector + '[disabled]' ) )
		return driverHelper.clickWhenClickable( this.driver, By.css( selector ) );
	}
	selectFreePlan() {
