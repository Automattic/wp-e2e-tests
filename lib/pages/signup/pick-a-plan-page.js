...
export default class PickAPlanPage extends BaseContainer {
	...
	selectFreePlan() {
		const oldMobileSelector = By.css( '.plan-features__mobile button.is-free-plan' );
		const freePlanButtonSelector = By.css( 'table button.is-free-plan' );
		return driverHelper.isElementPresent( this.driver, oldMobileSelector).then( ( present ) => {
			if ( present ) {
				return driverHelper.clickWhenClickable( this.driver, oldMobileSelector, this.explicitWaitMS );
			} else {
