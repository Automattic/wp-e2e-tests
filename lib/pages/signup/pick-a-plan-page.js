...
export default class PickAPlanPage extends BaseContainer {
	...
	selectFreePlan() {
		var freePlanButtonSelector;

		if ( screenSize === 'mobile' ) {
			freePlanButtonSelector = By.xpath( '//div[@class="plan-features__mobile"]//button[contains(., "Select Free")]' );
		} else {
			freePlanButtonSelector = By.xpath( '//table//button[contains(., "Select Free")]' );
		}
	...
	selectPersonalPlan() {
		...
		if ( screenSize === 'mobile' ) {
			personalPlanButtonSelector = By.css( '.plan-features__header.is-personal ~ .plan-features__actions .plan-features__actions-button' );
		} else {
			personalPlanButtonSelector = By.xpath( '//table/tbody/tr/td[count(//table/tbody/tr/td[contains(., "Personal")]/preceding-sibling::td)+1]//button' );
		}
	...
	selectPremiumPlan() {
		...
		if ( screenSize === 'mobile' ) {
			premiumPlanButtonSelector = By.css( '.plan-features__header.is-premium ~ .plan-features__actions .plan-features__actions-button' );
		} else {
			// Don't ask why I have to select Free here...xpath is terrible
			premiumPlanButtonSelector = By.xpath( '//table/tbody/tr/td[count(//table/tbody/tr/td[contains(., "Free")]/preceding-sibling::td)+1]//button' );
		}
	...
	selectBusinessPlan() {
		...
		if ( screenSize === 'mobile' ) {
			businessPlanButtonSelector = By.css( '.plan-features__header.is-business ~ .plan-features__actions .plan-features__actions-button' );
		} else {
			businessPlanButtonSelector = By.xpath( '//table/tbody/tr/td[count(//table/tbody/tr/td[contains(., "Business")]/preceding-sibling::td)+1]//button' );
		}
