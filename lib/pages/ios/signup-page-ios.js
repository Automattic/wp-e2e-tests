...
export default class SignupPage extends BaseContainerMobile {
	...
	verifyErrorPresent( errorText ) {
		...
		//return driver.wait( driver.findElement( errorSelector ).then( () => {
		return driver.wait( () => {
			return driver.findElement( errorSelector ).then( () => {
				return driverHelper.clickWhenClickableMobile( driver, errorOKButtonSelector ).then( () => {
					return true;
				}, () => {
					return false;
				} );
		...
		}, this.explicitWaitMS, `Timed out looking for '${errorText}'` );
	}
}
