...
export default class BusinessAddressPage extends BaseContainer {
	...
	selectAddBusinessAddress( ) {
		return driverHelper.clickWhenClickable( this.driver, By.css( '.card[data-e2e-type="business-address"] button' ) );
	}
	...
	enterBusinessAddressAndSubmit( name, street, city, state, zip, country ) {
		return driverHelper.waitTillPresentAndDisplayed( this.driver, By.css( '#name' ) )
			.then( () => {
