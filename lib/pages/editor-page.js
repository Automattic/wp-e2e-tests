...
export default class EditorPage extends BaseContainer {
	...
	uploadMedia( fileDetails ) {
		...
		self.driver.findElement( this.fileNameInputSelector ).sendKeys( newFile );
	...
	postSaved() {
		const driver = this.driver;
		const savedStatusSelector = by.css( '.editor-ground-control__save-status' );
		return driver.wait( until.stalenessOf( driver.findElement( savedStatusSelector ) ), this.explicitWaitMS, 'The post should started to save by now' ).then( () => {
			return driver.wait( until.elementLocated( savedStatusSelector ), this.explicitWaitMS, 'The post status should be saving by now' ).then( () => {
				return driver.wait( until.elementTextIs( driver.findElement( savedStatusSelector ), 'Saved' ), this.explicitWaitMS, 'Post should have been saved by now' );
			} );
		} );
	}
}
