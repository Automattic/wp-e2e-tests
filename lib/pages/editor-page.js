...
export default class EditorPage extends BaseContainer {
	...
	postSaved() {
		const savedStatusSelector = by.css( '.editor-ground-control__save-status[data-e2e-status=Saved]' );
		return this.driver.wait( until.elementLocated( savedStatusSelector ), this.explicitWaitMS, 'Post should have been saved by now' );
	}
}
