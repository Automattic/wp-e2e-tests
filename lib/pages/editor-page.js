...
export default class EditorPage extends BaseContainer {
	...
	selectImageByNumber( count ) {
		...
		driver.wait( until.elementLocated( by.className( 'media-library__upload-button' ) ), this.explicitWaitMS, 'Could not locate the media library upload button.' );
		driver.wait( until.elementsLocated( by.css( '.media-library__list-item:not(.is-placeholder)' ) ), this.explicitWaitMS, 'Could not locate image elements.' );
		driver.findElements( by.className( 'media-library__list-item' ) ).then( ( elements ) => elements[count].click() );
		return driver.wait( until.elementLocated( by.css( '.media-library__list-item.is-selected' ) ), this.explicitWaitMS, 'Could not locate the selected media item.' );
	}
	...
	insertPaymentButton( eyes, { title = 'Button', description = 'Description', price = '1.00', allowQuantity = true, email = 'test@wordpress.com' } = {} ) {
		driverHelper.clickWhenClickable( this.driver, by.css( '.mce-wpcom-insert-menu button' ) );
	...
	postIsScheduled() {
		return driverHelper.isElementPresent( this.driver, by.css( '.post-editor__inner .post-editor__content .editor-action-bar .editor-status-label.is-future' ) );
	}
}
