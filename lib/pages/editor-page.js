...
export default class EditorPage extends BaseContainer {
	...
	deleteMedia() {
		...
		driverHelper.waitTillPresentAndDisplayed( driver, deleteSelector );
		...
		driverHelper.waitTillPresentAndDisplayed( driver, acceptSelector );
	...
	dismissMediaModal() {
		const driver = this.driver;
		const selector = by.css( '.editor-media-modal__secondary-actions + .button' );
		driverHelper.waitTillPresentAndDisplayed( driver, selector );
		driverHelper.clickWhenClickable( driver, selector );
