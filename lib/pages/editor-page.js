...
export default class EditorPage extends BaseContainer {
	...
	waitUntilFeaturedImageInserted() {
		const self = this;
		self.driver.wait( until.elementLocated( by.css( '.post-editor__inner-content .editor-featured-image__preview' ) ), this.explicitWaitMS, 'Could not locate image in editor, check it is visible' );
		self.driver.wait( until.elementLocated( by.css( '[data-e2e-title="featured-image"] .editor-featured-image__preview img' ) ), this.explicitWaitMS, 'Could not locate image in editor, check it is visible' );
		return self.driver.switchTo().defaultContent();
	}
