...
export default class EditorPage extends BaseContainer {
	...
	uploadMedia( fileDetails ) {
		...
		driver.isElementPresent( by.css( '.mce-wpcom-insert-menu' ) ).then( ( menuPresent ) => {
			if ( menuPresent === true ) {
				driverHelper.clickWhenClickable( driver, by.css( '.mce-wpcom-insert-menu button.mce-open' ) );
				driverHelper.clickWhenClickable( driver, by.css( '.gridicons-add-image' ) );
			} else {
				driverHelper.clickWhenClickable( driver, by.css( '.mce-media' ) );
			}
		} );
