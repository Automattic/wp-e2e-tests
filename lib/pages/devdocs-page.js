	...
	screenshotAllElements( eyes ) {
		...
		return this.getAllDesignElementLinks().then( hrefs => {
			...
			for ( const href of hrefs ) {
				...
				flow.execute( () => this.isCurrentElementCompactable()
				.then( compactable => {
					if ( compactable ) {
						return this.getCurrentElementCompactButton()
							.then( button => {
								...
								return button.click().then( () => eyesScreenshot( driver, eyes, title + ' (Compact)', by.id( 'primary' ) ) );
							} );
					} } )
);
			}
