...
export default class DevdocsPage extends BaseContainer {
	...
	getAllDesignElementLinks() {
		...
		this.driver.findElements( this.designElementLinkSelector ).then( anchors => {
			let promiseArray = [];
			for ( const a of anchors ) {
				promiseArray.push(
					a.getAttribute( 'href' ).then( href => hrefs.push( href + '?debug=true' ) )
				);
			}

			webdriver.promise.all( promiseArray ).then( () => d.fulfill( hrefs ) );
		} );
	...
	hideMasterbar() {
		...
		return driverHelper.isElementPresent( driver, by.className( 'masterbar' ) ).then( present => {
			if ( present ) {
	...
	screenshotAllElements( eyes ) {
		...
		return this.getAllDesignElementLinks().then( hrefs => {
			...
			for ( const href of hrefs ) {
				...
				flow.execute( () => this.hideMasterbar() );
				// Hide the environment badge for simplicity
				flow.execute( () => this.hideEnvironmentBadge() );
				// Take the screenshot
				flow.execute( () => eyesScreenshot( driver, eyes, title, by.id( 'primary' ) ) );
				// Check for Compact button
				flow.execute( () =>
					this.isCurrentElementCompactable().then( compactable => {
						if ( compactable ) {
							return this.getCurrentElementCompactButton().then( button => {
								...
								if (
									...
									return driver
										...
										.perform()
										.then( () =>
											eyesScreenshot( driver, eyes, title + ' (Compact)', by.id( 'primary' ) )
										);
								}
