...
export default class ViewPostPage extends BaseContainer {
	...
	async leaveAComment( comment ) {
		...
		return await self.driver.wait(
			async function() {
				let present = await driverHelper.isElementPresent( self.driver, commentSubmittingSelector );
				return ! present;
			},
			self.explicitWaitMS,
	...
	async commentEventuallyShown( comment ) {
		...
		let element = await self.driver.wait(
			until.elementLocated( commentSelector ),
			this.explicitWaitMS
		);
		return !! element;
	}
