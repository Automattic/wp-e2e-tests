...
export default class ViewPostPage extends BaseContainer {
	...
	commentsVisible() {
		return driverHelper.isElementPresent( this.driver, By.css( '#respond' ) );
	}
	...
	categoryDisplayed() {
		return this.driver.findElement( By.css( 'a[rel="category tag"], a[rel="category"]' ) ).getText();
	}
	...
	paymentButtonDisplayed() {
		return driverHelper.isElementPresent( this.driver, By.css( '.jetpack-simple-payments-wrapper' ) );
	}
	...
	leaveAComment( comment ) {
		...
		return self.driver.wait( function() {
			return driverHelper.isElementPresent( self.driver, commentSubmittingSelector ).then( function( present ) {
				return !present;
			} );
