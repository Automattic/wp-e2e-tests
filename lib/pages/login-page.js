...
export default class LoginPage extends BaseContainer {
	...
	requestMagicLink( emailAddress ) {
		return driverHelper.clickWhenClickable( this.driver, By.css( 'a[data-e2e-link="magic-login-link"]' ) )
		.then( () => driverHelper.setWhenSettable( this.driver, By.css( '.magic-login__email-fields input[name="usernameOrEmail"]' ), emailAddress ) )
		.then( () => driverHelper.clickWhenClickable( this.driver, By.css( '.magic-login__form-action button.is-primary' ) ) )
		.then( () => driverHelper.waitTillPresentAndDisplayed( this.driver, By.css( '.magic-login__check-email-image' ) ) );
	}
