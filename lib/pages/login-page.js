...
export default class LoginPage extends AsyncBaseContainer {
	constructor( driver ) {
		super( driver, By.css( '#loginform, .wp-login__container' ) );
	}

	static async Visit( driver ) {
		const page = new LoginPage( driver );
		await page.visitInit( this.getLoginURL() );
	...
	async login( username, password ) {
		...
		let isWpcom = await driverHelper.isElementPresent( driver, wpcomLoginSelector );
		if ( isWpcom ) {
