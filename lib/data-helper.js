...
export function getJetpackSiteName() {
	...
	if ( host === 'CI' ) {
		const tunnelPrefix = process.env.TUNNEL_PREFIX ? process.env.TUNNEL_PREFIX : `wp-e2e-tests-jetpack-${process.env.CIRCLE_BUILD_NUM}`;
		return `${tunnelPrefix}.localtunnel.me`;
	}
