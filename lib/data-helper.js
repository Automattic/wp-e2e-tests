...
export function getExpectedFreeDotBlogAddress( searchTerm, dotBlogType ) {
	return [ `${searchTerm}.${dotBlogType}.blog` ];
}
...
export function getJetpackSiteName() {
	...
	if ( host === 'JN' ) {
		...
		return contents.split( ' ' )[0].replace( /^https?:\/\//, '' ).replace( /\/wp-admin/, '' );
	}
