 ...
 * @returns {object} value from conifg file
 */
...
export function getCalypsoLiveQuery() {
	if ( config.has( 'commitHash' ) ) {
		return 'hash=' + config.get( 'commitHash' );
	}
	return 'branch=' + config.get( 'branchName' );
}

export function getCalypsoURL( route = '' ) {
	let url;
	url = this.configGet( 'calypsoBaseURL' ) + '/' + route;
	if ( this.isRunningOnLiveBranch() ) {
		...
		url = url + '?' + this.getCalypsoLiveQuery();
	}
	return url;
}
