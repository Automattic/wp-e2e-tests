...
export function startBrowser( { useCustomUA = true, resizeBrowserWindow = true, acceptLanguage = null } = {} ) {
	...
	if ( config.has( 'sauce' ) && config.get( 'sauce' ) ) {
		...
		switch ( browser.toLowerCase() ) {
			case 'chrome':
				...
				if( acceptLanguage ) {
					options.setUserPreferences( {'intl.accept_languages': acceptLanguage } );
				}
				builder = new webdriver.Builder();
			...
			case 'firefox':
				...
				if ( acceptLanguage ) {
					profile.setPreference( 'intl.accept_languages', acceptLanguage );
				}
