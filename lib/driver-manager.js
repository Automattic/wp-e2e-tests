...
export function startBrowser() {
	...
	// If testing live branches, load the page first to set the session branch variable
	if ( config.has( 'liveBranch' ) && config.get( 'liveBranch' ) === 'true' ) {
		driver.wait( driver.get( config.get( 'calypsoBaseURL' ) + '/?branch=' + config.get( 'branchName' ) ), config.get( 'explicitWaitMS' ), 'Failed to load live branch' );
	}
