...
export function startApp() {
	...
	if ( config.has( 'sauce' ) && config.get( 'sauce' ) ) {
		...
		builder = new webdriver.Builder()
			...
			.withCapabilities( caps );
	} else if ( config.has( 'testdroid' ) && config.get( 'testdroid' ) ) {
		caps.set( 'testdroid_apiKey', config.get( 'testdroidAPIKey' ) );
		builder = new webdriver.Builder()
			.usingServer( 'http://appium.testdroid.com/wd/hub' )
			.withCapabilities( caps );
	} else {
...
//TODO: Debug logging
	...
	return driver.isElementPresent( webdriver.By.xpath( '//UIAImage[@name="icon-wp"]' ) ).then( function( atLoginScreen ) {
		if ( ! atLoginScreen ) {
