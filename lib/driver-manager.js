...
export function startBrowser() {
	...
	if ( config.has( 'liveBranch' ) && config.get( 'liveBranch' ) === 'true' ) {
		this.liveBranch = true
		this.branchName = config.get( 'branchName' );
		driver.wait( this.configureLiveBranch( driver ), config.get( 'explicitWaitMS' ), 'Failed to configure for live branch' );
	} else {
		this.liveBranch = false;
	}

	return driver;
}

export function configureLiveBranch( driver ) {
	if ( this.liveBranch ) {
		return driver.get( config.get( 'calypsoBaseURL' ) + '/?branch=' + this.branchName );
	}
}
...
export function clearCookiesAndDeleteLocalStorage( driver ) {
	...
	return driver.manage().deleteAllCookies().then( function() {
		return self.deleteLocalStorage( driver );
	} ).then( function() {
		return self.configureLiveBranch( driver );
	} );
}
