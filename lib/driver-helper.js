...
export function refreshIfJNError( driver, timeout = 1000 ) {
	const JNSiteError = by.xpath(
		"//pre[@class='error' and .='/srv/users/SYSUSER/log/APPNAME/APPNAME_apache.error.log']"
	);

	const refreshIfNeeded = t =>
		isEventuallyPresentAndDisplayed( driver, JNSiteError, t ).then( jnErrorDisplayed => {
			if ( jnErrorDisplayed ) {
				console.log( '===============\n+++++++++++++\n===============\n+++++++++++++\n' );
				console.log( ' jnErrorDisplayed! Reloading' );
				console.log( '===============\n+++++++++++++\n===============\n+++++++++++++\n' );
				return driver
					.navigate()
					.refresh()
					.then( () => refreshIfNeeded( timeout ) );
			}
			return true;
		} );

	return refreshIfNeeded( timeout );
}
