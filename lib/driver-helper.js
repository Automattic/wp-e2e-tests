...
export function waitForFieldClearableMobile( driver, selector ) {
	...
	self.scrollToFindElement( driver, selector ).then( ( element ) => {
		console.flog( 'found - ' + selector.value );
		return element.clear().then( function() {
			console.flog( 'cleared - ' + selector.value );
			d.fulfill( element );
		} );
	}, function() {
...
export function eyesScreenshot( driver, eyes, pageName, selector ) {
	...
	console.flog( `eyesScreenshot - ${pageName} - [${browserName}][${driverManager.currentScreenSize()}]` );
...
export function scrollToFindElement( driver, selector, { maxSwipes = 2 } = {} ) {
	...
	var swipeAndRetry = function( swipeNum ) {
		console.flog( 'swipeAndRetry - ' + swipeNum );
	...
	executeSearch = function( swipeNum ) {
		console.flog( 'executeSearch - ' + swipeNum + ' - ' + selector.value );
		driver.findElement( selector ).then( ( el ) => {
			console.flog( 'el found - ' + selector.value );
			return el.isDisplayed().then( function( visible ) {
				if ( visible ) {
					console.flog( selector.value + ' DISPLAYED!' );
					d.fulfill( el );
				} else {
			...
			console.flog( 'failed to find ' + selector.value );
			if ( swipeNum <= 0 ) {
				console.flog( 'EL NOT FOUND!' );
				d.reject( -1 );
			} else {
				swipeAndRetry( swipeNum );
			}
		} ).then( () => {
			console.flog( 'AT THE END' );
		} );
	};
