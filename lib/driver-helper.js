...
export async function getInfoMessageIfPresent( driver ) {
	const infoNoticeTextSelector = by.css( '.notice.is-info .notice__text' );

	try {
		let element = await driver.findElement( infoNoticeTextSelector );
		return element.getText();
	} catch ( e ) {
		return console.log( 'Info notice is not present.' );
	}
}
...
export function logPerformance( driver ) {
	if ( config.get( 'checkForConsoleErrors' ) === true ) {
		driver.manage().logs().get( 'performance' ).then( ( browserLogs ) => {
			browserLogs.forEach( ( browserLog ) => {
				var message = JSON.parse( browserLog.message ).message;
				if ( message.method === 'Network.responseReceived' || message.method === 'Network.requestWillBeSent' ) {
