...
export function followLinkWhenFollowable( driver, selector, waitOverride ) {
	...
	return driver.wait(
		() => driver.findElement( selector ).then(
			element => element.getAttribute( 'href' ).then(
				href => {
					driver.get( href );
					return true;
				},
...
export function waitTillPresentAndDisplayed( driver, selector, waitOverride ) {
	...
	return driver.wait(
		() => driver.findElement( selector ).then(
			element => element.isDisplayed().then(
				() => true,
				() => false
			),
			() => false
...
export function isEventuallyPresentAndDisplayed( driver, selector, waitOverride ) {
	...
	return driver
		.wait( () => driver.findElement( selector ).then(
			element => element.isDisplayed().then(
				() => true,
				() => false
			),
...
export function setCheckbox( driver, selector ) {
	...
	.then( checkbox => checkbox.getAttribute( 'checked' )
	.then( checked => {
		if ( checked !== 'true' ) {
...
export function waitTillNotPresent( driver, selector, waitOverride ) {
	...
	return driver.wait(
		() => this.elementIsNotPresent( driver, selector ),
				...
				if ( logs.length > 0 ) {
					forEach( logs, log => {
						...
						if (
							...
							driver.getCurrentUrl().then( url => SlackNotifier.warn( `Found console error: "${ log.message }" on url '${ url }'`, {
...
export function waitForInfiniteListLoad( driver, elementSelector, { numElements = 10 } = {} ) {
	return driver.wait( () => driver.findElements( elementSelector ).then( elements => elements.length >= numElements ) );
}

export function switchToWindow( driver, windowNunber ) {
	driver.getAllWindowHandles().then( result => driver
		.switchTo()
