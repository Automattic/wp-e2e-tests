...
export function setWhenSettableMobile( driver, selector, value, { secureValue = false } = {} ) {
	...
	// Does clear return the element?
	return self.waitForFieldClearableMobile( driver, selector ).then( ( element ) => {
	...
	return driver.wait( d.promise, explicitWaitMS * 20, `Timed out waiting for element with ${selector.using} of '${selector.value}' to be settable to: '${logValue}'` );
}
