...
export function setWhenSettable( driver, selector, value, {secureValue = false, pauseBetweenKeysMS = 0, assumeSuccess = false} = {} ) {
	...
	return driver.wait( function() {
		return driver.findElement( selector ).then( function( element ) {
			...
			return assumeSuccess || element.getAttribute( 'value' ).then( ( actualValue ) => {
...
export function switchToWindow( driver, windowNunber ) {
	driver.getAllWindowHandles().then( ( result ) => {
		driver.switchTo().window( result[windowNunber] ).then( () => {
			return true;
		} );
	} );
}
