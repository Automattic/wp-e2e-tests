...
export default class BaseContainer {
	...
	setABTestControlGroupsInLocalStorage( culture = 'en', flow = 'main', personalPlanValue = 'hide' ) {
		...
		const domainSuggestionVendorValue = 'namegen';
		const googleVoucherKey = 'googleVouchers_20160615';
		const googleVoucherValue = 'enabled';
		const personalPlanKey = 'personalPlan_20160627';
		const signupStoreKey = 'signupStore_20160927';
		const signupStoreValue = 'designTypeWithStore';
		const signupStoreBenchmarkingKey = 'signupStoreBenchmarking_20160927';
		const signupStoreBenchmarkingValue = 'pressable';
		const browserNotificationsKey = 'browserNotifications_20160628';
		const browserNotificationsValue = 'disabled';
		const browserNotificationsPreferencesKey = 'browserNotificationsPreferences_20160801';
		const browserNotificationsPreferencesValue = 'disabled';
		...
		const signupThemePreviewKey = 'signupThemePreview_20160727';
		const signupThemePreviewValue = 'hideThemePreview';
		const domainSuggestionPopoverKey = 'domainSuggestionPopover_20160809';
		const domainSuggestionPopoverValue = 'hidePopover';
		const signupCheckoutRedirectKey = 'signupCheckoutRedirect_20160826';
		const signupCheckoutRedirectValue = 'manual';
		const domainSuggestionsClickableRowKey = 'domainSuggestionClickableRow_20160802';
		const domainSuggestionsClickableRowValue = 'clickableButton';

		const expectedABTestValue = `{"${privacyCheckBoxKey}":"${privacyCheckBoxValue}","${freeTrialsSignupKey }":"${freeTrialsSignupValue}","${domainsWithPlanKey}":"${domainsWithPlanValue}","${guidedToursKey}":"${guidedToursValue}","${domainCreditsKey}":"${domainCreditsValue}","${domainSuggestionVendorKey}":"${domainSuggestionVendorValue}","${googleVoucherKey}":"${googleVoucherValue}","${personalPlanKey}":"${personalPlanValue}","${signupStoreKey}":"${signupStoreValue}","${browserNotificationsKey}":"${browserNotificationsValue}","${browserNotificationsPreferencesKey}":"${browserNotificationsPreferencesValue}","${skipFreePlansKey}":"${skipFreePlansValue}","${planFeaturesKey}":"${planFeaturesValue}","${signupThemePreviewKey}":"${signupThemePreviewValue}","${domainSuggestionPopoverKey}":"${domainSuggestionPopoverValue}","${signupCheckoutRedirectKey}":"${signupCheckoutRedirectValue}","${domainSuggestionsClickableRowKey}":"${domainSuggestionsClickableRowValue}","${signupStoreBenchmarkingKey}":"${signupStoreBenchmarkingValue}"}`;
