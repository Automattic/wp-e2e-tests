...
export default class BaseContainer {
	...
	checkForConsoleErrors() {
		this.driver.manage().logs().get( 'browser' ).then( function( logs ) {
			if ( logs.length > 0 ) {
				forEach( logs, ( log ) => {
					if ( log.message.indexOf( 'cast_sender.js' ) === -1 ) { // Ignore chrome cast errors in Chrome - http://stackoverflow.com/questions/24490323/google-chrome-cast-sender-error-if-chrome-cast-extension-is-not-installed-or-usi/26095117#26095117
						slackNotifier.warn( log.message, { suppressDuplicateMessages: true } );
					}
				} );
			}
		} );
