...
export default class BaseContainer {
	...
	setABTestControlGroupsInLocalStorage( culture = 'en' ) {
		...
		const domainCreditsValue = 'original';
		const domainSuggestionVendorKey = 'domainSuggestionVendor_20160614';
		const domainSuggestionVendorValue = 'namegen';

		const flow = '"main"';

		const expectedABTestValue = `{"${freeTrialKey}":"${freeTrialValue}","${freePlansDefaultKey}":"${freePlansDefaultValue}","${privacyCheckBoxKey}":"${privacyCheckBoxValue}","${freeTrialsSignupKey }":"${freeTrialsSignupValue}","${triforceSignupKey}":"${triforceSignupValue}","${domainsWithPlanKey}":"${domainsWithPlanValue}","${guidedToursKey}":"${guidedToursValue}","${domainCreditsKey}":"${domainCreditsValue}","${domainSuggestionVendorKey}":"${domainSuggestionVendorValue}"}`;
		...
		this.driver.getCurrentUrl().then( ( currentUrl ) => {
			...
			newUrl = newUrl.replace( 'domains-with-premium/survey', '' ); // remove the domains with plans survey path
