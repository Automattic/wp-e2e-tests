...
export function eyesOpen( driver, eyes, testEnvironment, testName ) {
	...
	if ( batchName !== '' ) {
		eyes.setBatch(
			batchName,
			`wp-e2e-tests-visdiff-${ global.browserName }-${ process.env.CIRCLE_BUILD_NUM }`
		);
	}
		...
		try {
			eyes.close( false ).then( testResults => {
				...
				if ( testResults.mismatches ) {
					message = `<!subteam^S0G7K98MB|flow-patrol-squad-team> Visual diff failed with ${
						testResults.mismatches
					} mismatches - ${ testResults.appUrls.session }`;
				} else if ( testResults.missing ) {
					message = `<!subteam^S0G7K98MB|flow-patrol-squad-team> Visual diff failed with ${
						testResults.missing
					} missing steps out of ${ testResults.steps } - ${ testResults.appUrls.session }`;
				} else if ( testResults.isNew ) {
					message = `<!subteam^S0G7K98MB|flow-patrol-squad-team> Visual diff marked as failed because it is a new baseline - ${
...
export function eyesScreenshot( driver, eyes, pageName, selector ) {
	if ( process.env.VISDIFF ) {
		...
		console.log(
			`eyesScreenshot - ${ pageName } - [${ browserName }][${ driverManager.currentScreenSize() }]`
		);

		// If there is a visible environment badge, hide it.
		driverHelper
			.isElementPresent( driver, By.css( '.environment-badge' ) )
			.then( isPresent => {
				if ( isPresent ) {
		...
		if ( process.env.EYESDEBUG ) {
			return driver.takeScreenshot().then( data => mediaHelper.writeScreenshot(
				data,
				() =>
					new Date().getTime().toString() +
					`-EYES-${ pageName }-${ driverManager.currentScreenSize() }`
			) );
		}
		...
		return driver.executeScript( 'document.activeElement.blur()' ).then( () => {
			if ( selector ) {
