...
test.describe( `[${ host }] Managing Domains: (${ screenSize }) @parallel`, function() {
	...
	test.describe(
		"Partially adding a domain to an existing site (Doesn't work in full until we work out a way to cancel the domain automatically)",
		() => {
			this.bailSuite( true );
			...
			const testDomainRegistarDetails = dataHelper.getTestDomainRegistarDetails(
				domainEmailAddress
			);
